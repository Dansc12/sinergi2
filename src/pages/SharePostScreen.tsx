...
const SharePostScreen = () => {
  ...
  // Get auto-generated name for placeholder
  const getAutoGeneratedPlaceholder = () => {
    const autoGenName = state?.contentData?.title as string;
    if (autoGenName && ["Morning Workout", "Afternoon Workout", "Evening Workout", "Night Workout", "Untitled Workout"].some(
      name => autoGenName.toLowerCase().trim() === name.toLowerCase()
    )) {
      return autoGenName;
    }
            ...
            {visibility !== "private" && (
              <div className="space-y-2">
                <Label htmlFor="workoutTitleDetails" className="flex items-center gap-2 text-sm">
                  Workout Name
                  {(isAutoGeneratedName || !workoutTitle.trim()) && (
                    <span className="text-xs text-destructive">(Custom name required)</span>
                  )}
                </Label>
                <Input
                  ...
                  className={`${isAutoGeneratedName || !workoutTitle.trim() ? "border-destructive/50" : ""}`}
                />
              </div>
            )}
        ...
        <AnimatePresence>
          {showDescription && (
            ...
            >
              <Textarea
                id="description"
                placeholder="Type your description. Share how awesome you are."
                value={description}
        ...
        {state?.contentData && Object.keys(state.contentData).length > 0 && (() => {
          ...
          <div className="mb-6">
            <motion.button
              ...
              className={`w-full p-4 rounded-xl bg-card border flex items-center justify-between ${
                workoutNeedsAttention ? "border-destructive/50" : "border-border"
              }`}
              whileTap={{ scale: 0.98 }}
            >
              <div className="flex items-center gap-2">
