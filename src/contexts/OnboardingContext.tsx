  ...
  const completeOnboarding = async () => {
    try {
      ...
      if (data.followedUserIds.length > 0) {
        ...
        const followInserts = data.followedUserIds.map((followeeId) => ({
          follower_id: user.id,
          followee_id: followeeId,
        }));
        await supabase.from("follows").insert(followInserts);

        const notificationInserts = data.followedUserIds.map((followeeId) => ({
          user_id: followeeId,
          type: "follow",
          title: "New follower",
          message: `${senderProfile?.display_name || "Someone"} started following you`,
